...
pub const PARALLAX_PIPELINE_HANDLE: HandleUntyped =
    HandleUntyped::weak_from_u64(PipelineDescriptor::TYPE_UUID, 3738901038901892983);
...
pub fn build_parallax_pipeline(shaders: &mut Assets<Shader>) -> PipelineDescriptor {
    PipelineDescriptor {
        depth_stencil: Some(DepthStencilState {
            format: TextureFormat::Depth32Float,
            depth_write_enabled: true,
...
pub mod node {
    ...
    pub const PARALLAX_SPRITE: &str = "parallax_sprite";
}
...
pub(crate) fn add_sprite_graph(
    ...
    graph.add_system_node(
        node::PARALLAX_SPRITE,
        RenderResourcesNode::<ParallaxSprite>::new(true),
    );
    ...
    pipelines.set_untracked(
        PARALLAX_PIPELINE_HANDLE,
        build_parallax_pipeline(shaders),
    );
}
